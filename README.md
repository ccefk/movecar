...
### Bark (iOS 推荐)
- 支持「紧急 / 重要 / 警告」通知级别。
- 可自定义通知音效，自定义推送通知Logo，甚至在**开启勿扰模式**时也能强提醒。
...
1. 扫描车上二维码，进入页面。
2. 填写留言（如「挡住出口了」）。
3. **允许获取位置**（若拒绝或失败（用户拒绝权限），系统依然允许发送通知）。
4. 点击「通知车主」，等待状态更新。
...
3. 点击「Edit code」，删除默认代码，将 **`movecar.js`** 全部内容粘贴进去，点击右上角「Deploy」。
...
- `BARK_LOGO_XIAOWANG`：该用户专属 Bark 推送通知自定义的Logo。
